...
case class PostSpot(title: String, content: String, latitude: Double, longitude: Double, postId: Long, image: String)
object PostSpot {
  implicit val jsonWrites = Json.writes[PostSpot]

  def apply(spot: Spot, post: Post):PostSpot ={
    PostSpot(post.title, post.content, spot.latitude, spot.longitude, post.id.get, post.image.getOrElse(""))
  }
}